...
The isolated agent controller requires Qwen's finding assessment before accepting a conclusive runtime verdict and its before/after fix assessment before accepting `fixed`. Deterministic runtime checks and immutable reproduction tests remain required. Validated review conclusions return to the coordinator; source snapshots and transient reasoning are not replayed as coordinator observations. Missing or dissenting reviews cannot be replaced by a manual source-review call. Reports preserve both phases and their exact test/context bindings.
