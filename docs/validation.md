...
## Initial audit deadline — September 8, 2026

A full project attempt stopped after 2,161.5 seconds during source reading: the initial 30-minute task deadline had received a single transient-provider retry allowance, but the four-hour specialist budget was not enabled until a review started. The attempt preserved 141 verified evidence records, made no edits to its 149 copied tracked files and did not reach local review or conclusive findings. Its failure remains recorded.

Connected security audits and tasks declaring required Foundation-Sec or Qwen reviews now receive the four-hour budget before inventory begins. Ordinary tasks keep their 30-minute initial limit and existing expansion on specialist invocation. The complete suite passed **336 tests in 356.79 seconds**. HTTP/Docker fixtures cover long initial reading, connected audit selection, ordinary-task expiry and rejection beyond four hours; they simulate elapsed time while executing actual controller/worker requests. A new live attempt starts the three reviewers before the full source-reading sweep; its unfinished results are not validated fixes or resumed conversation memory.
