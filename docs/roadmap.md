...
The isolated agent now adds a local Qwen coder/coordinator, executable tool loop, offline writable Docker workspace, pytest/Bandit, exported code/diffs, verified workspace continuation, and public Streamable HTTP MCP access. See [isolated-agent.md](isolated-agent.md) for the shipped contracts and boundaries.

Remaining work includes authenticated/stdio MCP, the CVE sidecar, cloud model disclosure/credential flows, network-enabled Nuclei/ZAP workers, authenticated API role matrices, general network discovery, abrupt-crash recovery hardening, more runtimes/lockfile formats, and the larger held-out model evaluation corpus. The first real engagement still requires an operator-selected target.
