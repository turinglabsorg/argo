...
Every new finding has a typed `verification` record, initially `pending`. Controller context includes a bounded queue and total/counts; `findings.list` returns 20 full entries per page, so compaction cannot discard pending IDs. The default action limit grows from 24 by eight per finding, capped at 1,024. An explicit `--max-steps` remains a strict 1–40 limit. Existing deadlines, source/export limits and model context budgets still apply.
