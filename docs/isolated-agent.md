...
Node can flatten the original exception inside a failed `assert.doesNotThrow` or `assert.doesNotReject` assertion into duplicate TAP diagnostic keys. The safe loader preserves the outer error's first keys so the nested exception does not overwrite its assertion identity. Uncaught runtime errors, setup failures and mixed assertion/runtime failures remain inconclusive.
...
Coordinator observations retain the test diagnostics, counts, test hashes and exact aggregate evidence ID. Repeated full source/support hash maps stay in durable evidence and mandatory Qwen review context. This reduces irrelevant repetition without changing model capacity or omitting source evidence from review. Rejected verdicts return the current finding's exact test ID and deterministic recovery guidance; unknown, stale or unrelated IDs remain invalid.
...
Five consecutive rejected actions stop the run as incomplete, including repeated premature completion attempts. A successful action resets the count. Reports index the rejected actions and recovery feedback in `action_errors`; saved evidence remains available. This guard does not reinterpret test results, execute corrective actions itself or replay completed tools.

An invalid coding reference returns its exact path and nearby existing workspace paths before any coder call or write. The coordinator must choose a corrected action; suggestions do not replace paths automatically. Malformed or schema-invalid provider output receives explicit action/parameters formatting guidance. Schema failures preserve expected constraints through authenticated provider requests without exposing the rejected response body.
...
Large review prompts use `shared-json-v1` when it saves space: repeated JSON values appear once in `shared_values`, with explicit `$argo_ref` references in the review document. The representation is lossless, including all support tests, lockfiles, diagnostics and before/after content. Complete manifests and stable support files precede changing verdict fields, with stable document ordering and shared-value references to preserve a reusable prompt prefix; this does not by itself prove a cache hit. The original expanded `review_context` and approval fingerprint stay in evidence; `review_input` records the wire encoding, payload hash and token estimate. Capacity checks use the serialized prompt plus schema, output reserve and headroom. Unique content that still exceeds capacity remains a blocker. Reference-like keys already present in structured evidence disable encoding to avoid ambiguous interpretation.
